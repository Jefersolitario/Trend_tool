...
def get_prices_data(start_utc, end_utc, country, price_selection, settings):
    ...
    if price_selection == 'day_ahead EXXA':
        ...
        price_data = get_vwap(country, product, start_utc, end_utc, vwap_delivery_1h, duration)
...
def get_freq(exit_price_data, entry_price_data):
    '''
    Get the frequency to find the most likely value
    '''
    freq_exit  = pd.infer_freq(exit_price_data.head(8).index.sort_values()) 
    freq_entry = pd.infer_freq(entry_price_data.head(8).index.sort_values()) 
...
def get_freq_old(exit_price_data, entry_price_data):
    ...
    if entry_price_data.index.freq != None:
        ...
        if freq_entry == None:
            ...
            freq_entry = pd.infer_freq(entry_price_data[0:end_row].index.sort_values())
    ...
    if exit_price_data.index.freq != None:
        ...
        if freq_exit == None:
            ...
            if freq_exit == None:
                freq_exit = pd.infer_freq(exit_price_data[0:8].index.sort_values()) 
                ## Fix bug IDA3 non continous time series needs small sampling to figure out the frequency
...
def _sharpe_ratio(result_data):

    """
    https://www.linkedin.com/pulse/reducing-trend-following-volatility-using-sharpe-ratio-justin-lee#:~:text=A%20simple%20way%20of%20overcoming,directional%20volatility%20that%20is%20desired.
    """
    Sharpe_FILTER = 1.7
    result_data = result_data.dropna()
    # daily_return = result_data["Profit"].groupby(result_data.index.date).sum()
    periods = len(result_data)**0.5
    Sharpe_ratio = periods*result_data.mean()/result_data.std()
    Periods = Sharpe_ratio[Sharpe_ratio.abs() > Sharpe_FILTER]
    Periods = Periods.reset_index()

    return Periods

def _sortino_ratio(result_data):
    """
    Calculate the Sortino Ratio for a dataset of returns.
    """
    Sortino_FILTER = 1.7  # Set an appropriate threshold as per your requirements
    result_data = result_data.dropna()

    risk_free_rate = 0 # Example: 2% annual risk-free rate

    # Calculate the excess returns
    excess_return = result_data - risk_free_rate

    # Calculate downside deviation (only consider negative returns)
    downside_deviation = (result_data[result_data < risk_free_rate] ** 2).mean() ** 0.5

    # Calculate the Sortino Ratio
    periods = len(result_data)
    Sortino_ratio = (periods ** 0.5) * excess_return.mean() / downside_deviation

    # Filter based on the Sortino Ratio
    Periods = Sortino_ratio[Sortino_ratio.abs() > Sortino_FILTER]
    Periods = Periods.reset_index()

    return Periods


def trend_finder(trend_table):

    PERC_FILTER = 70
    trend_table = trend_table.dropna()
    trend_sign = np.sign(trend_table)
    long_trend = trend_sign[trend_sign == 1]
    short_trend = trend_sign[trend_sign == -1]

    num_periods = trend_table.shape[0]

    long_trend_perc = 100*long_trend.sum()/num_periods
    short_trend_perc = 100*short_trend.sum()/num_periods

    long_trend_perc = long_trend_perc.loc[long_trend_perc.abs() > PERC_FILTER]
    short_trend_perc = short_trend_perc.loc[short_trend_perc.abs() > PERC_FILTER]
    short_trend_perc = short_trend_perc.reset_index()
    short_trend_perc['Side'] = -1
    long_trend_perc = long_trend_perc.reset_index()
    long_trend_perc['Side'] = 1

    periods = short_trend_perc._append(long_trend_perc)
    periods = periods[['Period' , 0,'Side']]
    periods = periods.sort_values(by = 0)
    periods.rename({0: '% Period trend'}, axis = 1, inplace = True)

    return periods

def _z_score(trend_table):
    # Identifying Outliers : https://medium.com/@marc.bolle/5-easy-ways-to-detect-outliers-in-python-b07639ba32d5
    # Z SCORE > 2 remove strong outliers
    data = trend_table.dropna().melt()
    data['z-score'] = (data['value'] - data['value'].mean())/data['value'].std()
    data = data.sort_values(by = 'z-score')
    outlier_periods = data.loc[data['z-score'].abs()>= 3]

    return outlier_periods

def create_st_plot(trends, trend_table, trend_periods, outlier_periods, entry_price):
    ...
    if st.button("Show binary spreads"):
        trend_table = np.sign(trend_table)
        fig = px.imshow(trend_table, color_continuous_scale=[[0, "red"], [1, "green"]], zmin= -1, zmax= 1 ,text_auto=True)
    elif 'vol' in entry_price:
    ...
    automatic_trend = trends.loc[trends['Period'].isin(trend_periods['Period'])]
    outliers = trends.loc[trends['Period'].isin(outlier_periods['Period'].unique())]
    fig_short = px.line(trends, x=trends.index, y='spread cumpnl hourly', color= 'Period', title='Trend by hour')
    fig_auto_trend = px.line(automatic_trend, x=automatic_trend.index, y='spread cumpnl hourly', color= 'Period', title='Automatic Trend identification')
    fig_outliers = px.line(outliers, x=outliers.index, y='spread', color= 'Period', title='Automatic Outliers identification')
...
def trend_tracker_main():
    ...
    country = st.selectbox('Select a country', settings['kpi_settings']['country_list'], index= 2) # 0change list order to FR
    ...
    if 'vol' in entry_price:
        ...
        exit_price = st.selectbox('Select an exit price', settings['kpi_settings']['price'], index= 3) 
    ...
    trend_table = create_table(trends)
    # trend_periods = trend_finder(trend_table)
    trend_periods = _sortino_ratio(trend_table)
    # trend_periods = _sharpe_ratio(trend_table)
