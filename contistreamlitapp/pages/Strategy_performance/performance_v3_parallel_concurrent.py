...
from pages.Strategy_performance.performance_v3_summary import process_strategy, create_stacked_plot
...
# from contistreamlitapp.pages.Strategy_performance.performance_v3_summary import process_strategy, create_stacked_plot
...
def proces_data(data, strategy, entry_price, exit_price, settings):
    ...
    data['Volume'] = data['Volume']*data['Signal'] ### correct bug: volume for not trading signals
...
def strategy_perf():
    ...
    if url_params:
        strategy = url_params.get('strategy')
        if strategy not in settings.keys():
            st.error(f'{strategy} Strategy not found')

        else:

            st.title(f'{strategy} Performance')
        ...
        tab1, tab2 = st.tabs(["🗃 Performance", "📈 Performance Summary "])
        with tab1:
            st.title('Strategy Performance')
            end_date = pd.to_datetime('today') #- pd.Timedelta(days=1)
            start_date = end_date - pd.Timedelta(days=30)#8

            strategy = st.selectbox('Select a strategy', settings['kpi_settings']['strategy_list'], index= 16)#2
        ...
        with tab2:
            ...
            start_date =  pd.to_datetime('2024-01-02') #end_date - pd.Timedelta(days=30)
