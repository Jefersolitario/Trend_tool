...
from pages.Strategy_performance.utilities.data import get_spot_prices, get_strategy_old, get_strategy, get_actuals, get_ts_history, get_vwap_index,  get_vwap, get_ts_db
from pages.Strategy_performance.utilities.kpi_metrics import Kpi
# from contistreamlitapp.pages.Strategy_performance.utilities.data import get_spot_prices, get_strategy_old, get_strategy, get_actuals, get_ts_history, get_vwap_index,  get_vwap, get_ts_db
...
def get_data(start_date, end_date, strategy, settings):
    ...
    new_schema_strat_list = ['at_solar_strategy', 'nl_solar_signal']
    ...
    ## put another condition to read from a new source mongo db!!!!
        ...
        if signal is None or signal.empty:
            ...
            st.stop()
        signal.rename({'Direction_probability': 'Probability'}, axis= 1, inplace= True)
    ...
    entry_price = get_prices_data(start_date, end_date, entry_price_id, entry_name, strategy, settings)
    exit_price = get_prices_data(start_date, end_date, exit_price_id, exit_name, strategy, settings)
...
def strategy_perf():
    ...
    start_date = end_date - pd.Timedelta(days=30)#8

    strategy = st.selectbox('Select a strategy', settings['kpi_settings']['strategy_list'], index= 0)
    # strategy = 'strat_francewind_rte'
